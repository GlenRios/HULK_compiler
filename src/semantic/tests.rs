...
fn base_in_method_ok() {
    ...
    check_no_err!(
        p,
        |e| matches!(e, &SemanticError::UndefinedVariable { ref name, .. } if *name == "base")
    );
}
...
fn protocol_inherited_methods_count_ok() {
    ...
    let p = program_with_decls(
        vec![
            ...
            type_decl("Child", vec![], Some("Base"), vec![]),
        ],
        let_(
...
fn protocol_parent_type_conforms_child_also_conforms_ok() {
    ...
    let p = program_with_decls(
        vec![
            ...
            type_decl("Eagle", vec![], Some("Bird"), vec![]),
        ],
        let_(
